...
public final class ThreadGroup {
    ...
    private ThreadGroup(java.lang.ThreadGroup nativeGroup, String name) {
        this.backingGroup = nativeGroup;
        this.name = name;
        java.lang.ThreadGroup nativeParent = nativeGroup.getParent();
        if (nativeParent != null) {
            ThreadGroup wrappedParent = groupMap.get(nativeParent);
            if (wrappedParent == null) {
                wrappedParent = new ThreadGroup(nativeParent, new String(nativeParent.getName()));
            }
            this.parent = wrappedParent;
        } else {
            this.parent = null;
        }
        groupMap.put(nativeGroup, this);
    }
    ...
    public static ThreadGroup getSystemThreadGroup(java.lang.ThreadGroup nativeGroup) {
        ...
        if (wrapped == null) {
            wrapped = new ThreadGroup(nativeGroup, new String(nativeGroup.getName()));
        }
        return wrapped;
    }
    ...
    public int enumerate(ThreadGroup[] list, boolean recurse) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (wrapped == null) {
                wrapped = new ThreadGroup(nativeList[i], new String(nativeList[i].getName()));
            }
            list[added++] = wrapped;
        }
        return added;
